...
class GUI:
    ...
    def connect(self):
        try:
            self.db = pymysql.connect(host="localhost",user="root",passwd = "Mithu@123",db="world")
    ...
    def loginPage(self):
        self.rootWin = Tk()
        self.rootWin.title("Login Page")
        self.cardPage = Toplevel()
        self.cardPage.withdraw()
        self.ticketsPage = Toplevel()
        self.ticketsPage.withdraw()
        response = urllib.request.urlopen("https://thumbs.gfycat.com/EsteemedExemplaryHypsilophodon-max-1mb.gif")
        ...
        Label(self.rootWin, text="Train Ticket Booking", font=("High Tower Text", 18),bg='light blue').grid(row=1,column=0,columnspan=5,sticky=E+W)
    ...
    def registerPage(self):
        ...
        self.regWin.title("New Registration")
        Label(self.regWin, text="User Registration", font=("Bell MT", 18),bg='light blue').grid(row=0,column=0,columnspan=4,sticky=E+W)
        ...
        self.officialRegisterButton = Button(self.regWin,text="Create a new user",width = 15,command=self.registerNew)
    ...
    def registerNew(self):
        ...
        if self.pass1 != self.pass2:
            messagebox.showwarning("Warning","The Passwords Do Not Match")
            return
        elif self.user=="" or self.pass1=="" or self.pass2=="":
            messagebox.showwarning('Field cannot be empty',"Fill in all the fields")
            return
        ...
        if cursor1!=0:
           messagebox.showwarning("Enter a different username","Username Already in Database")
        elif cursor2!=0:
           messagebox.showwarning("This email already has an account.","Email Already in Database")
        else:
            ...
            database.commit()
            messagebox.showinfo("New User Registered", "User Registered Successfully")
    ...
    def customerPage(self):
        ...
        self.pic.grid(row = 0, column = 0, columnspan=4)
        self.custPage.title("Menu Page")
        self.viewButton = Button(self.custPage,text="View Train Schedule",width = 20,command=self.viewPage)
        self.viewButton.grid(row=1,column=1)
        self.makeButton=Button(self.custPage,text="Make a New Reservation",width = 20,command=self.makeReservationPage)
        self.makeButton.grid(row=2,column=1)
        self.updateButton=Button(self.custPage,text="Update a Reservation",width = 20,command=self.updateReservation)
        self.updateButton.grid(row=3,column=1)
        self.cancelButton = Button(self.custPage,text="Cancel a Reservation",width = 20,command=self.cancelReservation)
        self.cancelButton.grid(row=4,column=1)
        self.giveButton = Button(self.custPage,text="Give Feedback",width = 20,command=self.makegiveReviewPage)
        self.giveButton.grid(row=5,column=1)
        self.viewReviewButton = Button(self.custPage,text="View Feedback", width=20,command=self.makeViewReviews)
        self.viewReviewButton.grid(row=6,column=1)
        self.schoolButton = Button(self.custPage,text="Add School Info",width = 20,command=self.schoolPage)
        self.schoolButton.grid(row=7,column=1)
        self.logoutButton=Button(self.custPage,text="Log Out", width =9,command=self.logout)
    ...
    def schoolPage(self):
        ...
        Label(self.schoolPage,text="Must End with .ac.in").grid(row=2,column=0)
        self.backButton=Button(self.schoolPage,text="Go Back", width =5,command=self.back)
    ...
    def managerPage(self):
        ...
        Label(self.managerWindow, text="Menu Page", font=("Bell MT", 18),bg='light blue').grid(row=1, column=0, columnspan=2)
        ...
        Button(self.managerWindow, text="Go Back", command=self.closeManager).grid(row=3, column=0, sticky=W)
    ...
    def revenueReport(self):
        ...
        for i in includedMonths:
            ...
            cursor.execute("Select SUM(Reservation1.TotalCost) FROM Reservation1 INNER JOIN Reserves On Reservation1.ReservationID=Reserves.ReservationID WHERE month(Reserves.DepartureDate)="+str(i))
            for x in cursor:
                revenues.append(x[0])
        for i in range(len(revenues)):
            if revenues[i] == None:
                revenues[i] = 0
            Label(self.revenue, text="Rs"+str("%.2f" %round(revenues[i], 2))).grid(row=i+2, column=1, sticky=W)
        Label(self.revenue, text="Month", font=("Bell MT", 14, "bold")).grid(row=1, column=0, sticky=W)
        Label(self.revenue, text="Revenue", font=("Bell MT", 14, "bold")).grid(row=1, column=1, sticky=W)
        for i in range(len(includedMonths)):
            Label(self.revenue, text=monthNames[includedMonths[i]-1]).grid(row=2+i, column=0, sticky=W)
        Button(self.revenue, text="Go Back", command=self.closeRev).grid(row=5, column=0)
    ...
    def popularTrain(self):
        ...
        try:
            ...
            pass
        cursor.execute("CREATE VIEW TopMonths AS SELECT month(DepartureDate) AS Month, TrainNumber, Count(R.ReservationID) AS  NumReservations FROM Reserves INNER JOIN Reservation1 R WHERE R.isCancelled = FALSE AND R.ReservationID=Reserves.ReservationID GROUP BY month(DepartureDate) LIMIT 3;")
        cursor.execute("CREATE VIEW TopTrains AS SELECT month(DepartureDate) AS Month, TrainNumber, Count(R.ReservationID) AS  NumReservations FROM Reserves INNER JOIN Reservation1 R WHERE isCancelled = FALSE AND R.ReservationID=Reserves.ReservationID GROUP BY TrainNumber LIMIT 3;")
        ...
        while i < 3 and len(monthInfo) > i:
            ...
            i += 1
        trainReportMonth = Label(self.popularWindow, text="Month", font=("Bell MT", 14, "bold"))
        trainReportName = Label(self.popularWindow, text="Train", font=("Bell MT", 14, "bold"))
        trainReportFrequency = Label(self.popularWindow, text="Frequency", font=("Bell MT", 14, "bold"))
        closeTrainReport = Button(self.popularWindow, text="Go Back", command=self.closePopularTrains)
    ...
    def submit(self):
        ...
        if schoolEmail2[-6:] == ".ac.in":
          ...
          messagebox.showwarning("Success","Student information verified")
        else:
          messagebox.showwarning("Error","Enter a School Email")
    ...
    def viewPage(self):
        ...
        self.pic.grid(row = 0, column = 0, columnspan=4)
        self.searchButton=Button(self.viewPage, text="Search Trains",width=15,command=self.viewSchedule)
    ...
    def viewSchedule(self):
        ...
        if cursor !=0:
          ...
          cursor.execute("SELECT TrainNumber,ArrivalTime,DepartureTime,Name,Location FROM Stop1 NATURAL JOIN Station WHERE TrainNumber = %s ORDER BY ArrivalTime",(self.viewEntry.get()))
          stops = cursor.fetchall()
          Button(self.viewSched, text = "Go Back", command = self.backView).grid(row=2+len(stops), column= 0)
    ...
    def makeReservationPage(self):
        ...
        self.reservationPage.title("New Reservation")
        self.pic = Label(self.reservationPage, image = self.photo)
        self.pic.grid(row = 0, column = 0, columnspan=2)
        Label(self.reservationPage, text="Search Train", font=("Bell MT", 18),bg='light blue').grid(row=1,column=0,columnspan=2,sticky=E+W)
        ...
        self.findTrainsButton = Button(self.reservationPage, text="Find list of Trains", command = self.findTrains)
    ...
    def findTrains(self):
        if self.departureVar.get() == "" or self.arrivalVar.get() == "" or self.departureVar.get()==self.arrivalVar.get():
            ...
            try:
                ...
                messagebox.showwarning("Error","Incorrect departure date. Please enter date in MM/DD/YYYY format")
                return
    ...
    def selectDeparturePage(self):
        ...
        self.pic.grid(row = 0, column = 0, columnspan=4)
        Label(self.findTrainsPage, text="Select Departure", font=("Bell MT", 18),bg='light blue').grid(row=1,column=1,columnspan=2,sticky=E+W)
        ...
        self.c.execute("CREATE OR REPLACE VIEW GoodArrivals AS SELECT TrainNumber, ArrivalTime FROM Stop1 NATURAL JOIN Station WHERE Name = '" + self.arrStopName + "'")
        self.c.execute("CREATE OR REPLACE VIEW GoodDeparts AS SELECT TrainNumber, DepartureTime FROM Stop1 NATURAL JOIN Station WHERE Name = '" + self.depStopName + "'")
        ...
        for i in range(len(goodTrains)):
            ...
            Radiobutton(self.findTrainsPage, text="Rs"+str(goodTrains[i][4]), variable=self.selectedTicket, value=(str(i)+","+"1")).grid(row=3+i,column=2)
            Radiobutton(self.findTrainsPage, text="Rs"+str(goodTrains[i][5]), variable=self.selectedTicket, value=(str(i)+","+"2")).grid(row=3+i,column=3)
        Button(self.findTrainsPage, text="Go Back", command=self.backToReservationPage).grid(row=3+len(goodTrains),column=0,sticky=E+W)
    ...
    def makeTravelExtrasPage(self):
        ...
        self.travelExtrasPage.title("Baggage and Passenger Info")
        self.pic = Label(self.travelExtrasPage, image = self.photo)
        self.pic.grid(row = 0, column = 0, columnspan=2)
        Label(self.travelExtrasPage, text="Baggage and Passenger Info", font=("Bell MT", 18),bg='light blue').grid(row=1,column=0,columnspan=2,sticky=E+W)
        Label(self.travelExtrasPage, text="Number of Baggage").grid(row=2,column=0,sticky=W)
        Label(self.travelExtrasPage, text="Maximum 4 Bags(2 Free of Charge.Next 2 are Rs.300 per bag.)").grid(row=3,column=0,columnspan=2,sticky=E+W)
        ...
        Button(self.travelExtrasPage, text="Go Back", command=self.backToFindTrains).grid(row=5,column=0,sticky=E+W)
    ...
    def setupTicket(self):
        if self.baggageNumVar.get() == "":
            messagebox.showwarning("Error","Please enter number of baggages")
        else:
    ...
    def makeTicketsPage(self):
        ...
        Label(self.ticketsPage, text="Make Reservation", font=("Bell MT", 18),bg='light blue').grid(row=1,column=0,columnspan=9,sticky=E+W)
        ...
        if len(self.ticketsList) != 0:
            ...
            if a[0][0] == 1:
                ...
                Label(self.ticketsPage, text="No Student Discount available.").grid(row=4+len(self.ticketsList),column=0,columnspan=2,sticky=W)
            ...
            Label(self.ticketsPage, text="Pay using Card").grid(row=6+len(self.ticketsList),column=0)
            ...
            if x == 0:
                self.lastFourList= ["No Card details Found"]
            else:
                for card in cards:
            ...
            Button(self.ticketsPage, text="Add Card details", command=self.makeCardPage).grid(row=6+len(self.ticketsList),column=2,sticky=W)
            Button(self.ticketsPage, text="Add another train ticket", command=self.newTrainPressed).grid(row=7+len(self.ticketsList),column=0,columnspan=2)
            Button(self.ticketsPage, text="Go Back", command=self.backFromMakeReservation).grid(row=8+len(self.ticketsList),column=0)
            Button(self.ticketsPage, text="OK", command=self.payForReservation).grid(row=8+len(self.ticketsList),column=2)
        else:
            Button(self.ticketsPage, text="Go Back", command=self.backFromMakeReservation).grid(row=4,column=0)

    def payForReservation(self):
        if self.cardVar.get() != "" and self.cardVar.get() != "No Card details Found":
            ...
            if expD < datetime.datetime.now().date():
                ...
                a = self.c.execute("SELECT MAX(ReservationID) FROM Reservation1")
                b = self.c.fetchall()
                ...
                self.c.execute("INSERT INTO Reservation1 VALUES(%s,%s,%s,%s,%s)",(self.username,self.reservationNumber,False,cardNo,self.totalcost))
            ...
            messagebox.showwarning("Error","No Card details selected for payment.")

    def createConfirmationPage(self):
        self.ticketsPage.withdraw()
        self.confirmationPage = Toplevel()
        self.confirmationPage.title("Ticket Confirmation")
        self.pic = Label(self.confirmationPage, image = self.photo)
        self.pic.grid(row = 0, column = 0, columnspan=3)
        Label(self.confirmationPage, text="Ticket Confirmation", font=("Bell MT", 18),bg='light blue').grid(row=1,column=0,columnspan=3,sticky=E+W)
        ...
        Label(self.confirmationPage, text="Have a nice trip! Please save this reservation ID for future requirements.").grid(row=3,column=0,columnspan=3,sticky=E+W)
        Button(self.confirmationPage, text="Go Back to Menu page",command=self.backFromConfirmation).grid(row=4,column=0,columnspan=3,sticky=E+W)
        for label in self.ticketsPage.winfo_children():
            label.grid_forget()
    ...
    def makeCardPage(self):
       ...
       self.pic.grid(row = 0, column = 0, columnspan=2)
       Label(self.cardPage, text="Add Card details").grid(row=1,column=0)
       ...
       Label(self.cardPage, text="Expiry Date (MM/YYYY)").grid(row=5,column=0)
       ...
       Label(self.cardPage, text="Delete Card details").grid(row=1,column=2)
       Label(self.cardPage, text="Card Number").grid(row=2,column=2)
       Button(self.cardPage, text="Go Back", command=self.backFromCardPage).grid(row=7,column=0)
       x = self.c.execute("SELECT CardNumber FROM PaymentInfo WHERE Username = '" + self.username + "'")
       self.cardList2 = []
       cards = self.c.fetchall()
       if x == 0:
           self.addDelLastFourList= ["No Card details Found"]
       else:
           for card in cards:
       ...
       self.cardVar2.set("No Card details Found")
       ...
       self.submitPaymentInfoButton=Button(self.cardPage, text="OK",width=10,command=self.submitPaymentInfo)
       self.submitPaymentInfoButton.grid(row=6,column=0)
       self.submitDeleteInfoButton=Button(self.cardPage,text="OK",width=10, command=self.submitDeleteInfo)
    ...
    def submitDeleteInfo(self):
       db=self.connect()
       cursor=db.cursor()
       if self.addDelLastFourList[0] == "No Card details Found" or self.cardVar2.get() == "No Card details Found":
           ...
           messagebox.showinfo("Success!","Card details successfully deleted!")
           for label in self.cardPage.winfo_children():
                label.grid_forget()
           self.makeCardPage()
    ...
    def updateReservation(self):
        ...
        self.UpdateBackButton=Button(self.updateReservationPage, text="Go Back",width=10,command=self.updateBack)
    ...
    def updateReservationSearch(self):
        ...
        num=self.c.execute("SELECT * FROM Reservation1 NATURAL JOIN Customer WHERE ReservationID =%s AND Username =%s AND isCancelled = FALSE",(self.updateIDEntry1,self.username))
        self.updateIDEntry1=int(self.updateIDEntry.get())
        if num !=0:
            self.updateReservationPage.withdraw()
            ...
            self.help33List=[]
            for item in trains:
                self.c.execute("CREATE or Replace VIEW ArrivalTime AS SELECT TrainNumber, ArrivalTime FROM Stop1 NATURAL JOIN Station WHERE TrainNumber=%s AND Name=%s",(item[0],item[1]))
                self.c.execute("CREATE OR REPLACE VIEW DepartureTime AS SELECT TrainNumber, DepartureTime FROM Stop1 NATURAL JOIN Station WHERE TrainNumber = %s AND Name = %s",(item[0],item[2]))
    ...
    def nextUpdate2(self):
            ...
            self.c.execute("Select totalCost from Reservation1 where ReservationID=%s",self.updateIDEntry1)
            price=self.c.fetchall()
            self.price=float(price[0][0]+50)
            Label(self.nextUpdatePage2,text="Change Total cose").grid(row=5,column=1)
    ...
    def submitChange(self):
        self.c.execute("Update Reservation1 SET totalCost=%s where ReservationID=%s",(self.price,self.updateIDEntry1))
    ...
    def makegiveReviewPage(self):
        ...
        self.giveReviewPage.title("Give Feedback")
        ...
        Label(self.giveReviewPage,text="comments").grid(row=3,column=0)
        ...
        self.submitReviewButton=Button(self.giveReviewPage, text="OK",width=10,command=self.submitReview)
        self.submitReviewButton.grid(row=4,column=0)
        self.giveReviewBackButton=Button(self.giveReviewPage, text="Go Back",width=10,command=self.giveReviewBack)
    ...
    def makeViewReviews(self):
       ...
       self.viewReviews.title("View Feedback")
       ...
       self.viewReviewBackButton=Button(self.viewReviews,text="Go Back",width=10, command=self.viewReviewBack)
    ...
    def makeviewReviewNextPage(self):
       ...
       if cursor1 !=0:
           ...
           Label(self.viewReviewNext, text="comments").grid(row=1,column=1)
           ...
           for x in a:
               ...
               rowcount=rowcount+1
           self.viewReviewBack2Button=Button(self.viewReviewNext,text="Back to Menu Page",width=20, command=self.viewReviewBack2)
    ...
    def submitReview(self):
        ...
        if cursor5==0:
            ...
            cursor3.execute("INSERT into review(TrainNumber,Username,ReviewNum,Comment,rating) Values (%s,%s,%s,%s,%s)",(self.trainNumberReview.get(),self.username,self.reviewNum,self.commentReview.get(),self.reviewVar.get()))        
    ...
    def cancelReservation(self):
        ...
        cursor.execute("SELECT Reserves.DepartureDate, Reserves.DepartsFrom, Reserves.ArrivesAt, Reserves.Class, Reservation1.ReservationID, Reservation1.totalCost FROM Reservation1 INNER JOIN Reserves ON Reservation1.ReservationID=Reserves.ReservationID WHERE Reservation1.Username='"+self.username+"' AND Reservation1.IsCancelled=FALSE AND DATEDIFF(Now(), Reserves.DepartureDate) < -1")
        ...
        for i in range(len(registrations)):
            for j in range(7):
                if j == 5:
                    Label(self.cancelPage, text="Rs."+str("%.2f" % round(float(registrations[i][j]), 2))).grid(row=i+2, column=j, sticky=W)
                elif j < 5:
        ...
        confirmButton = Button(self.cancelPage, text="Confirm cancellation", command=lambda: self.cancelConfirmation(cancelList, registrations))
        confirmButton.grid(row=len(registrations)+3, column=1)
        backButton = Button(self.cancelPage, text="Go Back", command=self.closeCancel)
    ...
    def cancelConfirmation(self, cancelList, registrations):
        ...
        for i in range(len(cancelList)):
            if cancelList[i].get() == 1:
                ...
                cursor.execute("SELECT CASE WHEN DATEDIFF(Now(), Reserves.DepartureDate) < -7 THEN (SELECT CASE WHEN (Reservation1.totalCost * 0.8) - 50.0 > 0 THEN( Reservation1.totalCost * 0.8) - 50.0  ELSE 0 END) ELSE (SELECT CASE WHEN Reservation1.totalCost * 0.5 - 50 > 0 THEN Reservation1.totalCost * 0.5 - 50 ELSE 0 END) END AS `Refund`, Reservation1.ReservationID, Reservation1.totalCost FROM Reservation1 INNER JOIN Reserves ON Reservation1.ReservationID=Reserves.ReservationID WHERE Reservation1.ReservationID="+str(regID))
        for x in cursor:
            newCancelList.append(x)
        Label(self.confirmationPage, text="Click yes to cancel your reservation?").grid(row=1, column=0, columnspan=2)
        Label(self.confirmationPage, text="ID").grid(row=2, column=0)
        Label(self.confirmationPage, text="Refund amount:").grid(row=2, column=1)
        for i in range(len(newCancelList)):
            Label(self.confirmationPage, text=newCancelList[i][1]).grid(row=i+3, column=0)
            Label(self.confirmationPage, text="Rs"+str("%.2f" % round(float(newCancelList[i][0])))).grid(row=i+3, column=1)
    ...
    def cancelConfirm(self, cancelList, registrations):
        cursor=self.connect().cursor()
        for i in range(len(cancelList)):
            cursor.execute("UPDATE Reservation1 SET IsCancelled=TRUE WHERE ReservationID='"+str(cancelList[i][1])+"'")
            cursor.execute("UPDATE Reservation1 SET totalCost="+str(float(cancelList[i][2])-float(cancelList[i][0]))+" WHERE ReservationID='"+str(cancelList[i][1])+"'")
            messagebox.showwarning("Success", "Your train ticket has been cancelled successfully!")
        self.connect().commit()
